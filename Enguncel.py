...
def telegram_uyari_gonder(mesaj):
    try:
        # Eğer kendi telegram_bot nesneni kullanacaksan burada onu kullan.
        # Bot token ve chat id global'den alınmalı.
        OCR_BOT_TOKEN = "7638711968:AAGNI42aL66NtvkCVt9uezCnxKzX6w1mVSo"
        OCR_CHAT_ID = "2005146572"
        bot = Bot(token=OCR_BOT_TOKEN)
        bot.send_message(chat_id=OCR_CHAT_ID, text=mesaj)
    except Exception as e:
        print("Telegram uyarı gönderilemedi:", e)

def windows_balon_goster(mesaj, baslik="Alarm!"):
    try:
        notification.notify(
            title=baslik,
            message=mesaj,
            app_name="Knight Takip",
            timeout=10
        )
    except Exception as e:
        print("Windows balon bildirimi hatası:", e)
...
# --- ENVANTER VERİLERİ ---
envanter_yedek_path = os.path.join(desktop, "envanter_verileri.json")

# Program başında envanter_verileri yükle
envanter_verileri = {}
if os.path.exists(envanter_yedek_path):
    try:
        with open(envanter_yedek_path, "r", encoding="utf-8") as fenv:
            envanter_verileri = json.load(fenv)
    except Exception as e:
        print("Envanter verileri yüklenirken hata:", e)

# ... (Diğer listeler ve sabitler)
KAMP_LIST = ["1", "2", "3", "4", "5"]
...
# --- ENVANTER YAZMA/YÜKLEME FONKSİYONLARI ---
def envanter_yedekle():
    try:
        with open(envanter_yedek_path, "w", encoding="utf-8") as f:
            json.dump(envanter_verileri, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print("Envanter verileri kaydedilemedi:", e)


# Örneğin, envanter ekleme/düzenleme/silme işlemlerinden sonra:
# envanter_yedekle() fonksiyonunu çağır.

# Program başlarken yüklenecek zaten, ekstra yüklemeye gerek yok.
...
def envanter_buton_tiklandi():
    ...
    for c, t, w in [
        ("item", "Eşya", 180),
        ("kalan", "Kalan Adet", 80),
        ("toplam", "Toplam Adet", 80),
        ("yuzde", "Kalan %", 80),
        ("saatlik_tuketim", "Saatlik Tüketim", 100)
    ]:
        tree_env.heading(c, text=t)
        tree_env.column(c, anchor="center", width=w)
    tree_env.pack(fill="both", expand=True)

    btn_frame = tk.Frame(envanter_pencere)
    btn_frame.pack(fill="x", pady=5)

    # Giriş ID ile doğru karakteri bul
    secili_id = None
    ...
    # --- Envanter dosyasını yükle
    global envanter_verileri
    if os.path.exists(envanter_yedek_path):
        try:
            with open(envanter_yedek_path, "r", encoding="utf-8") as f:
                envanter_verileri = json.load(f)
        except Exception as e:
            print("envanter_verileri.json okunamadı:", e)
            envanter_verileri = {}
    ...
    # --- Envanter kaydet fonksiyonu
    def kaydet_envanter_dosyaya():
        envanter_verileri[secili_id] = veriler
        try:
            with open(envanter_yedek_path, "w", encoding="utf-8") as f:
                json.dump(envanter_verileri, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print("envanter_verileri.json'a yazılamadı:", e)
    ...
    def envanter_kaydet():
        kaydet_envanter_dosyaya()
        messagebox.showinfo("Başarılı", "Envanter kaydedildi!")
    ...
    btn_kaydet = tk.Button(btn_frame, text="Kaydet", command=envanter_kaydet)
    ...
    def guncelle():
        ...
        for i, esya in enumerate(veriler):
            ...
            # --- KRİTİK VE BİTTİ BİLDİRİMİ ---
            key = f"{secili_id}-{esya['item']}"
            # 0 OLDUĞUNDA BİTTİ ALARMI
            if int(kalan) == 0 and (key+"-bitti") not in bildirim_gonderilen_esyalar:
                mesaj = f"Alarm!\n{secili} karakterinde '{esya['item']}' TAMAMEN BİTTİ!"
                telegram_uyari_gonder(mesaj)
                windows_balon_goster(mesaj)
                bildirim_gonderilen_esyalar.add(key+"-bitti")
            # %20'nin altına ilk inişte KRİTİK
            elif 0 < yuzde < 20 and (key+"-kritik") not in bildirim_gonderilen_esyalar:
                mesaj = f"Kritik!\n{secili} karakterinde '{esya['item']}' %20'nin altına düştü!"
                telegram_uyari_gonder(mesaj)
                windows_balon_goster(mesaj)
                bildirim_gonderilen_esyalar.add(key+"-kritik")
            # Yüzde tekrar %20 üstüne çıkarsa tekrar atabilsin diye resetle
            elif yuzde >= 20:
                bildirim_gonderilen_esyalar.discard(key+"-bitti")
                bildirim_gonderilen_esyalar.discard(key+"-kritik")

        envanter_pencere.after(1000, guncelle)

    guncelle()  # Canlı güncellemeyi başlat
...
def envanter_penceresi_ac(index):
    ...
    # --------- ENVANTER BİTTİ UYARI ---------
    for esya, miktar in envanter.items():
            key = f"{char_id}-{esya}"
            try:
                if int(miktar) == 0 and son_envanter_uyarilari.get(key) != True:
                    mesaj = f"{char_id} karakterinde '{esya}' TAMAMEN BİTTİ!"
                    telegram_uyari_gonder(mesaj)
                    windows_balon_goster(mesaj)
                    son_envanter_uyarilari[key] = True
                elif int(miktar) > 0:
                    son_envanter_uyarilari[key] = False
            except:
                continue
...
def yedekle():
    # Ana veritabanı yedeği (car_listesi ve gizlenenler)
    ...
    # Envanter verilerini de kaydet!
    try:
        with open(envanter_yedek_path, "w", encoding="utf-8") as fenv:
            json.dump(envanter_verileri, fenv, ensure_ascii=False, indent=2)
        log_yaz("Envanter yedeği alındı", envanter_yedek_path)
    except Exception as e:
        print("Envanter yedeği alınırken hata:", e)
...
def guncelle_liste():
    global car_listesi  # <-- Fonksiyonun en başına!

    # print("car_listesi:", car_listesi)
    # print("Toplam karakter:", len(car_listesi))
    ...
    # Eğer son_mana_uyarilari dict'in tanımı fonksiyon dışında yapılmadıysa, onu da global olarak çağır:
    global son_mana_uyarilari
    ...
    for car in car_listesi:
        ...
        # --------- MANA BİTTİ UYARI BLOĞU ---------
        char_id = car.get("Karakter İsmi", "?")
        if toplam_mana > 0 and kalan_mana == 0 and son_mana_uyarilari.get(char_id) != True:
            mesaj = f"{char_id} karakterinin manası BİTTİ!"
            telegram_uyari_gonder(mesaj)
            windows_balon_goster(mesaj)
            son_mana_uyarilari[char_id] = True
        elif kalan_mana > 0:
            son_mana_uyarilari[char_id] = False  # Tekrar uyarı verebilir hale gelir
...
# ... (sonrası ve diğer fonksiyonlar burada devam ediyor)
...
def genie_saniyeye_cevir(genie_str):
    try:
        ...
        return 0
def tum_degisklikleri_kaydet():
    # Karakter kayıtları
    try:
        with open(manuel_yedek_dosya, "w", encoding="utf-8") as f:
            json.dump({"aktifler": car_listesi, "gizlenenler": gizlenenler}, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print("Manuel yedek alınamadı:", e)
    ...
    # (İstersen başka kritik yapıların yedeğini de ekleyebilirsin)

    messagebox.showinfo("Başarılı", "Tüm değişiklikler başarıyla kaydedildi!")
...
def kaydet():
    ...
    # Eğer güncelleme ise, eski kaydı al
    eski = None
    if secili_index is not None and 0 <= secili_index < len(car_listesi):
        eski = car_listesi[secili_index]
    else:
        eski = {}

    # Sadece güncellenen alanları değiştir!
    car = dict(eski)  # Eskiyi kopyala, yenilerle üstünü yaz
    car.update({
        ...
        "PC": pc_bilgi.get(),
        "Not": not_kutu.get(),
        "Genie": genie_str if genie_str != eski.get("Genie", "") else eski.get("Genie", ""),
        "Genie Canli": genie_canli_str if genie_str != eski.get("Genie", "") else eski.get("Genie Canli", ""),
        # Sadece Genie değiştiyse, last_update güncellenir
        "Genie_last_update": now_ts if genie_str != eski.get("Genie", "") else eski.get("Genie_last_update", now_ts),
        "Premium": int(premium_var.get()) if premium_var.get().isdigit() else eski.get("Premium", 0),
        "Premium_last_update": now_ts if premium_var.get() != str(eski.get("Premium", "")) else eski.get("Premium_last_update", now_ts),
        "Toplam Mana": toplam_mana_var.get(),
        "Saatlik Mana Tüketimi": saatlik_mana_var.get(),
        "Mana Seviyesi": mana_seviyesi_otomatik,
        "mana_last_update": now_ts,  # Burası zorunlu reset!
        "gizli_mi": eski.get("gizli_mi", False),
        "Envanter": eski.get("Envanter", {"Hammer": 20, "Potion": 10, "Gold": 5000})
    })
...
def sag_tik_menu(event):
    try:
        ...
        if rowid:
            ...
            menu.add_command(label="Seç", command=lambda: sec_komut(rowid))
            menu.add_command(label="Gizle", command=lambda: gizle_komut(rowid))
            menu.add_command(label="Envanter Takip", command=lambda: envanter_takip_komut(rowid))
            menu.tk_popup(event.x_root, event.y_root)
    except Exception as e:
        print("Sağ tık hata:", e)

def sec_komut(rowid):
    tree.selection_set(rowid)
    tree.focus(rowid)
    secili_karakter_guncelle(None)  # BURADA HEM LABEL HEM ID GÜNCELLENİR

def gizle_komut(rowid):
    # Sağ tık ile seçilen satırı gizle
    tree.selection_set(rowid)
    tree.focus(rowid)
    gizle_secili_kayit()
    secili_karakter_guncelle(None)
...
def envanter_takip_komut(rowid):
    # Sağ tık ile seçilen satırın envanterini aç
    tree.selection_set(rowid)
    tree.focus(rowid)
    index = tree.index(rowid)
    envanter_penceresi_ac(index)
    secili_karakter_guncelle(None)
    print("Envanter Takip Açıldı!")
...
buttons = [
    ...
    # --- BURAYA EKLEDİK ---
    ("Yaptığım Tüm Değişiklikleri Kaydet", lambda: tum_degisklikleri_kaydet(), "#1877f2", "white")
]
...
def secili_karakter_guncelle(event):
    ...
    if selected:
        ...
        secili_karakter_id = values[0] if len(values) > 0 else None
    else:
        secili_karakter_var.set("Seçili Karakter: Yok")
        secili_karakter_id = None



# Sadece bu eventi bağla
tree.bind("<<TreeviewSelect>>", secili_karakter_guncelle)  # Satıra tıklayınca otomatik günceller
